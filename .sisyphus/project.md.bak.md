...
- [ ] Self-hosted PostgreSQL + TimescaleDB configured
...
- [ ] Real-time updates via TimescaleDB subscriptions or self-hosted realtime

**Phase 3 - Prediction**:
- [ ] TimesFM inference service (FastAPI)
- [ ] Prediction overlay on signals
- [ ] Configurable parameters per signal
- [ ] Playground prediction testing
...
- [ ] Event generation from model predictions
- [ ] Event timeline UI
...
- Supabase managed services (full self-hosted)
...
Wave 1.2 (After 1.1):
├── T1.5: Initialize self-hosted PostgreSQL + TimescaleDB
...
├── T2.1: Create signal management tables
├── T2.2: Implement signal CRUD API
└── T2.3: TimescaleDB hypertable setup

Wave 2.2 (Database connector):
├── T2.4: DB connector configuration UI
├── T2.5: Query builder for signal selection
└── T2.6: CDC or polling implementation

Wave 2.3 (MQTT connector):
├── T2.7: MQTT broker setup in Docker
├── T2.8: Topic browser UI
├── T2.9: MQTT subscription management
└── T2.10: Message parsing and storage

Wave 2.4 (File upload + visualization):
├── T2.11: File upload API (CSV, JSON)
├── T2.12: Signal preview component
├── T2.13: Real-time chart visualization
└── T2.14: Playwright E2E for import flows
```
...
├── T3.1: Setup Python FastAPI service
├── T3.2: Integrate TimesFM model
├── T3.3: Inference endpoint implementation
└── T3.4: ClearML agent setup

Wave 3.2 (Prediction UI):
├── T3.5: Prediction overlay on charts
├── T3.6: Parameter configuration UI
└── T3.7: Playground workspace UI

Wave 3.3 (Integration):
├── T3.8: Connect signal data to inference
├── T3.9: Real-time prediction updates
└── T3.10: E2E prediction flow test
```
...
- [ ] 1.1. Initialize Next.js + shadcn project
...
- [ ] 1.2. Setup Git repo with conventional commits
...
- [ ] 1.3. Setup Docker Compose base structure
  ...
  **What to do**:
  - Create `docker-compose.yml` with:
    ...
    - GoTrue or self-hosted auth placeholder
  ...
  **References**:
  - `https://docs.timescale.com/timescaledb/latest/install/docker/` - TimescaleDB Docker
...
- [ ] 1.5. Initialize self-hosted PostgreSQL + TimescaleDB
...
- [ ] 1.6. Design database schema (org, users, signals)

  **What to do**:
  - Create SRTD migration structure in `database/migrations/`
  - Design tables:
  ...
  **QA Scenarios**:
  ...
  Scenario: Migrations apply cleanly
    Tool: Bash
    Steps:
      1. Run `psql -h localhost -p 5432 -U postgres -d kairo -f database/migrations/001_initial_schema.sql`
      2. Check for errors
...
- [ ] 1.7. Implement RLS policies
  ...
  **References**:
  - `https://www.postgresql.org/docs/current/ddl-rowsecurity.html` - RLS docs
  ...
  **QA Scenarios**:
  ...
  Scenario: Anon user cannot see organizations
    ...
    Steps:
      1. Run `psql -h localhost -p 5432 -U anon -d kairo -c "SELECT count(*) FROM organizations;"`
    Expected Result: Returns 0 or permission denied
  ...
  Scenario: User sees only their org
    Tool: Bash
    Steps:
      1. Get user token from auth service
  ...
  **Commit**: YES
  ...
  - Files: `database/migrations/002_rls_policies.sql`
  - Pre-commit: None

---

- [ ] 1.8. Create SRTD migration structure

  **What to do**:
  - Install and configure SRTD (https://github.com/t1mmen/srtd)
  - Create migration templates for each functional area:
    - `database/functions/auth/` - user management
    - `database/functions/signals/` - signal CRUD
    - `database/functions/events/` - event handling
    - `database/functions/notifications/` - notification channels
    - `database/functions/audit/` - audit logging
  ...
  **Commit**: YES
  ...
  - Files: `database/srtd.yaml`, `database/migrations/`, `database/functions/`
  - Pre-commit: None
...
- [ ] 1.9. Implement authentication (OAuth + email)

  **What to do**:
  - Configure GoTrue or self-hosted auth with:
  ...
  **References**:
  - `https://github.com/supabase/gotrue` - Self-hosted auth
  ...
  **Commit**: YES
  ...
  - Files: `frontend/app/(auth)/`, `database/migrations/auth.sql`
  - Pre-commit: `pnpm lint`

---

- [ ] 1.10. User management CRUD

  **What to do**:
  - Create user management API (RPC functions)
  ...
  **References**:
  - PostgreSQL RPC functions documentation
  ...
  **Commit**: YES
  ...
  - Files: `frontend/app/settings/users/`, `database/functions/users/`
  - Pre-commit: None
...
- [ ] 1.11. Organization management
  ...
  **References**:
  - Auth hooks for organization creation
  ...
  **Commit**: YES
  ...
  - Files: `frontend/app/settings/organization/`, `database/functions/orgs/`
  - Pre-commit: None

---

- [ ] 1.12. Audit logging system
  ...
  **Commit**: YES
  ...
  - Files: `database/migrations/audit.sql`, `frontend/app/audit/`
  - Pre-commit: None
...
- [ ] 1.13. Role-based permissions
  ...
  **Commit**: YES
  ...
  - Files: `database/functions/permissions.sql`, `frontend/lib/permissions.ts`
  - Pre-commit: None

---

- [ ] 1.14. Test auth flow E2E
...
- [ ] 1.15. Sync designs to Stitch, get approval
...
- [ ] F3. Full E2E test suite — `unspecified-high` + `playwright`
- [ ] F4. Performance testing — `deep`
...
# Backend
psql -c "SELECT 1"  # PostgreSQL healthy
